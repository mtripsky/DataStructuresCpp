...
SCENARIO( "testing Queue enqueue with int as data" ) {
    GIVEN( "constructing  queue of size 2" ) {
        ...
        WHEN( "enqueue 1, 2 and then 3" ) {
            ...
            THEN( "size should be 2 and front element should be 1 and last enqueue should be false and queue should be full" ) {
                REQUIRE( sut.size() == 2);
                REQUIRE( sut.front() == 1);
                REQUIRE(result1);
                REQUIRE(result2);
...
SCENARIO( "testing Queue dequeue with int as data" ) {
    GIVEN( "constructing  queue of size 2" ) {
        ...
        WHEN( "enqueue three times from queue containing 1 and 2 (in order)" ) {
            ...
            THEN( "size should be 0 and front should throw exception and last enqueue should be false and queue should be empty" ) {
                REQUIRE( sut.size() == 0);
