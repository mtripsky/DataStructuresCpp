...
SCENARIO( "testing Stack push with int as data" ) {
    GIVEN( "constructing stack of size 2" ) {
        ...
        WHEN( "push 1, 2 and then 3" ) {
            ...
            THEN( "size should be 2 and top element should be 2 and last push should be false and stack should be full" ) {
                REQUIRE( sut.size() == 2);
                REQUIRE( sut.top() == 2);
                REQUIRE(result1);
                REQUIRE(result2);
...
SCENARIO( "testing Stack pop with int as data" ) {
    GIVEN( "constructing stack of size 2" ) {
        ...
        WHEN( "pop three times from queue containing 1 and 2 (in order)" ) {
            ...
            THEN( "size should be 0 and top should throw exception and last pop should be false and stack should be empty" ) {
                REQUIRE( sut.size() == 0);
